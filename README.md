...
```bash
# Build (Linux example; adjust -arch for your GPU)
nvcc -O3 -std=c++17 -arch=sm_70 \
     sim.cpp gel.cu fluid.cu coupling.cu \
     gel_kernels.cu fluid_kernels.cu coupling_kernels.cu \
     -o gLSM_fluid_3D

# Run with defaults (two gels, 10,000 steps)
./gLSM_fluid_3D
```

The binary writes `*.dat` files into the working directory every 1000 steps. See [Output Artefacts](#output-artefacts) for naming conventions.
...
1. **Initialization**
   - `Gel` and `Fluid` constructors allocate host/device buffers, zero or seed them, and upload parameter structs to constant memory.
   - Initial gel geometry and chemistry fields are populated from analytic formulas embedded in the code.

2. **Main Time Step (`runstep` iterations)**
   ...
   - *Immersed Boundary Sub-iterations:* For each sub-cycle the code packs Lagrangian boundary data, interpolates Eulerian velocities to the gel nodes (`k_ibm_interp`), updates gel mechanics with the interpolated velocities, and spreads forces back to the fluid grid (`k_ibm_spread`).

3. **Fluid Update**
   - Executes LBM collide/stream via `k_collide`/`k_stream` kernels, applies IBM body forces, and updates macroscopic fields (`rho`, `u`, concentration scalars).

4. **Output**
   - Every 1000 steps, gel and fluid fields are written to disk for visualization and diagnostics.
